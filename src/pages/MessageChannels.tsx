import { MessageSquare, CheckCircle, AlertCircle, TrendingUp, Settings } from 'lucide-react';
import { BarChart, Bar, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer } from 'recharts';

const channels = [
  { name: '微信公众号', icon: '💬', status: 'healthy', sends: 12450, openRate: 71.2, failRate: 0.8, color: '#10b981', followers: '23.4万' },
  { name: '企业微信', icon: '🏢', status: 'healthy', sends: 5680, openRate: 84.5, failRate: 0.3, color: '#6366f1', followers: '1.2万' },
  { name: '短信', icon: '📱', status: 'warning', sends: 8920, openRate: 45.8, failRate: 3.2, color: '#f59e0b', followers: '-' },
  { name: '小程序', icon: '📲', status: 'healthy', sends: 4230, openRate: 62.3, failRate: 1.1, color: '#0891b2', followers: '8.7万' },
  { name: 'APP消息', icon: '📱', status: 'offline', sends: 320, openRate: 22.1, failRate: 12.4, color: '#dc2626', followers: '0.3万' },
];
    ...
    <div className="p-6 space-y-5 fade-in">
            ...
            <div key={ch.name} className="metric-card p-4">
              ...
              <div className="space-y-1">
                <div className="flex justify-between text-xs">
                  ...
                  <span className="font-medium" style={{ color: ch.color }}>{ch.sends.toLocaleString()}</span>
                </div>
                <div className="flex justify-between text-xs">
                ...
                <div className="flex justify-between text-xs">
                  ...
                  <span style={{ color: ch.failRate > 3 ? '#ef4444' : '#64748b' }}>{ch.failRate}%</span>
                </div>
              </div>
              <div className="mt-3 progress-bar">
                <div className="progress-fill" style={{ width: `${ch.openRate}%`, background: ch.color }} />
              </div>
            </div>
          );
        })}
        ...
        <div className="col-span-2 glass-card p-5">
          ...
          <ResponsiveContainer width="100%" height={220}>
            <BarChart data={channelTrend}>
              ...
              <Bar dataKey="微信" fill="#10b981" radius={[2, 2, 0, 0]} stackId="a" />
              <Bar dataKey="短信" fill="#f59e0b" radius={[0, 0, 0, 0]} stackId="a" />
              <Bar dataKey="企业微信" fill="#6366f1" stackId="a" />
              <Bar dataKey="小程序" fill="#0891b2" radius={[2, 2, 0, 0]} stackId="a" />
            </BarChart>
          </ResponsiveContainer>
          <div className="flex items-center justify-center gap-6 mt-2">
            {[['微信', '#10b981'], ['短信', '#f59e0b'], ['企业微信', '#6366f1'], ['小程序', '#0891b2']].map(([n, c]) => (
        ...
        <div className="space-y-4">
          <div className="glass-card p-5">
            ...
            <div className="space-y-3">
              {channels.map(ch => {
                const s = statusStyle[ch.status];
                return (
                  <div key={ch.name} className="flex items-center gap-3 py-2"
                    style={{ borderBottom: '1px solid rgba(71,85,105,0.15)' }}>
                    <span className="text-base">{ch.icon}</span>
                    <div className="flex-1">
                      <div className="flex items-center justify-between">
                        <span className="text-xs font-medium text-slate-300">{ch.name}</span>
                      ...
                      <div className="text-xs text-slate-500">失败率 {ch.failRate}%</div>
                    </div>
                  </div>
                );
              })}
          ...
          <div className="glass-card p-5">
            ...
            <div className="space-y-2">
              {['微信公众号AppID', '企业微信CorpID', '短信签名', '小程序AppID'].map(item => (
      ...
      <div className="glass-card overflow-hidden">
        <div className="p-4 flex items-center gap-3" style={{ borderBottom: '1px solid rgba(71,85,105,0.2)' }}>
          ...
          <span className="text-sm font-semibold text-slate-200">消息模板管理</span>
            ...
            {templates.map((t, i) => (
              <tr key={i} className="table-row-hover" style={{ borderBottom: '1px solid rgba(71,85,105,0.1)' }}>
                ...
                <td className="px-4 py-3">
                  <span className="tag-blue text-xs px-2 py-0.5 rounded">{t.channel}</span>
                </td>
                <td className="px-4 py-3 text-sm text-slate-300">{t.uses}</td>
                <td className="px-4 py-3">
                  <div className="flex items-center gap-1.5">
                    {t.status === 'active' ? <CheckCircle size={12} className="text-emerald-400" /> : <AlertCircle size={12} className="text-slate-500" />}
                    <span className="text-xs" style={{ color: t.status === 'active' ? '#10b981' : '#64748b' }}>
                ...
                <td className="px-4 py-3">
                  <div className="flex gap-2">
                    ...
                    <button className="text-xs text-slate-400 hover:text-slate-300">预览</button>
