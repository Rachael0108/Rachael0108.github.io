...
export default function Sidebar({ currentPage, onPageChange }: SidebarProps) {
  ...
  return (
    <aside
      ...
      style={{
        background:
          "linear-gradient(180deg, var(--medical-sidebar-start) 0%, var(--medical-sidebar-end) 100%)",
        borderRight: "1px solid var(--medical-border-strong)",
        position: "relative",
        flexShrink: 0,
      }}
    >
      ...
      >
        <div
          ...
          style={{
            background:
              "linear-gradient(135deg, var(--medical-button), var(--medical-blue-400))",
          }}
        >
        ...
        {!collapsed && (
          <div>
            ...
            <div className="text-xs text-sky-200">精准触达平台</div>
          </div>
        )}
      </div>
      ...
      {!collapsed && (
        ...
        >
          ...
          <div className="flex items-center gap-2 mt-1">
            ...
            <span className="text-xs text-sky-100">今日生成内容 247 条</span>
          </div>
        </div>
      )}
        ...
        {menuItems.map((item) => {
          ...
          return (
            ...
            >
              <Icon
                size={18}
                className={`flex-shrink-0 ${isActive ? "text-sky-100" : "text-sky-200/80"}`}
              />
              {!collapsed && (
                <div className="overflow-hidden">
                  <div
                    className={`text-sm font-medium truncate ${isActive ? "text-white" : "text-sky-100"}`}
                  >
                    {item.label}
                  </div>
                  <div className="text-xs text-sky-200/70 truncate">
                    {item.desc}
                  </div>
                </div>
              )}
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <div className="overflow-hidden">
              ...
              <div className="text-xs text-sky-200/70 truncate">运营管理员</div>
            </div>
          </div>
        </div>
      )}
